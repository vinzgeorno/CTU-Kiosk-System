...
exports.buildTransactionDetails = buildTransactionDetails;
const facilities_1 = require("../config/facilities");
function buildTransactionDetails(input) {
    ...
    for (const [rawCategoryCode, rawQuantity] of Object.entries(input.quantities)) {
        const categoryCode = rawCategoryCode;
        ...
        if (!allowedCategoryCodes.has(categoryCode)) {
            throw new Error(`Category '${categoryCode}' is not allowed for facility '${facility.code}' (${facility.name}).`);
        }
        const category = facility.categories.find((item) => item.code === categoryCode);
        if (!category) {
            throw new Error(`Invalid category '${categoryCode}' for facility '${facility.code}' (${facility.name}).`);
        }
        breakdown.push({
